...
function cambiarPagina(numeroPagina) {
  ...
  document.querySelectorAll('.entrevista-page-btn').forEach(btn => {
    btn.addEventListener('click', function() {
      ...
      if (accion === 'anterior') {
        nuevaPagina = Math.max(1, parseInt(document.querySelector('.entrevista-page-btn.active').dataset.pagina) - 1);
      } else if (accion === 'siguiente') {
        nuevaPagina = Math.min(2, parseInt(document.querySelector('.entrevista-page-btn.active').dataset.pagina) + 1);
      } else {
  ...
  // Función para agregar filas al psicograma
